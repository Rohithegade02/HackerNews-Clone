                                        ...
                                        <div className='flex items-center' key={index}>
                                            ...
                                            <div className='flex flex-col'>
                                                ...
                                                <div className='flex gap-3 items-center ml-[30px]'>
                                                    ...
                                                    <div>
                                                        {votes[item.id] &&
                                                            <div><Button sx={{ color: ' #8E9EB8' }} fontSize={'15px'} fontWeight={400} onClick={() => handleFalseVote(item.id)} >Unvote</Button></div>}
                                                    </div>
                                                    <div>
                                                        <Typography sx={{ color: ' #8E9EB8' }} fontSize={'15px'} fontWeight={400}> {unixTimestampToMinutesAgo(item.time)} </Typography>
                                                    </div>
                                                    <div>
                                                        <Button sx={{ color: ' #8E9EB8' }} fontSize={'15px'} fontWeight={400} onClick={() => handleHideClick(item.id)}>Hide</Button>
                                                    </div>
                                                    <div className='mb-[2px]'>
                                                        {Array.isArray(item.kids) ? (
                                                            <Link
                                                                to={`/comments/${item.id}`}
                                                                state={{ kids: item.kids, title: item.title, score: item.score, by: item.by }}
                                                            >
